...
public:
  ...
  dim(dim) {
    normalize=false;
    l2space = new hnswlib::TanimotoSpace<dist_t, data_t>(dim);
    appr_alg = NULL;
    ep_added = true;
    index_inited = false;
  ...
  std::string space_name = "tanimoto";
  int dim;
  size_t seed;
  size_t default_ef;
  ...
  ~Index() {
    ...
    py::dict getAnnData() const { /* WARNING: Index::getAnnData is not thread-safe with Index::addItems */
      ...
      unsigned long level0_npy_size = appr_alg->cur_element_count * appr_alg->size_data_per_element_;
      ...
      return py::dict(
                      ...
                      "setbit_offset"_a=appr_alg->setbit_offset_,
    ...
    static Index<dist_t, data_t> * createFromParams(const py::dict d) {
      ...
      Index<dist_t, data_t> *new_index = new Index<dist_t, data_t>(dim_);
...
inline void register_index_class(py::module &m, std::string className) {
    py::class_<Index<dist_t, data_t>>(m, className.c_str())
        ...
        .def("get_ann_data", &Index<dist_t, data_t>::getAnnData)
